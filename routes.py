...
def index():
    ...
    return render_template('index.html',categories=categories,pname=pname,cname=cname,price=price)
...
def delete_cart(id):
    ...
    if cart.user_id != session['user_id']:
        flash('You\'re not authorized to access the cart')
        return redirect(url_for('cart'))
...
def checkout():
    ...
    for cart in carts:
        ...
        if cart.product.quantity < cart.quantity:
            flash('Product {cart.product.name} is out of stock')
